...
<div class="container-fluid" style="margin-top: 10px;">
    <div class="row">
		...
		<nav aria-label="breadcrumb">
              		<ol class="breadcrumb">
                		<li class="breadcrumb-item"><a href="{{ route('categoryList') }}">Danh mục</a></li>
                			@foreach ($stack as $index => $element)
                    				@if ($index == count($stack) - 1)
                        				...
                        				@endphp
                    				@endif
                    				<li class="breadcrumb-item"><a href="{{ route('postOfCate', ['unsigned_title'=>changeUrl($element)]) }}">{{ $element->name }}</a></li>
                			@endforeach

                		<li class="breadcrumb-item active" aria-current="page">{{ $stack[0]->name }}</li>
              		</ol>
            	</nav>
