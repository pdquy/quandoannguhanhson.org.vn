		              ...
		              <ul class="spost_nav">
		                  <li>
		                      <div class="media wow fadeInDown animated" style="visibility: visible; animation-name: fadeInDown;">
		                        ...
		                        <div class="media-body"> <a href="{{ route('postDetail', ['unsigned_title'=>$post->unsigned_title , 'id'=>$post->id]) }}" class="catg_title">{{ cutTitle($post->title,10) }}</a> </div>
		                      </div>
		                    </li>
		              </ul>
		            </div>
					...
					<div class="col-sm-6 col-lg-6 col-md-6 col-xl-6" style="height: 370px">
						...
						>
						   <a href="{{ route('postDetail', ['unsigned_title'=>changeUrl($value->loaitin), 'id'=>$value->id]) }}" title="{{ $value->title }}"><img src="{{ asset('uploads/images/posts') }}/{{ $value->image }}" alt="{{ $value->title }}" style="width: 100%">    <div class="time-l">
						    ...
						    </a><div class="info-news" style="text-transform: uppercase;"><a href="{{ route('postDetail', ['unsigned_title'=>changeUrl($value->loaitin), 'id'=>$value->id]) }}" title="{{ cutTitle($value->title,10) }}">
						        </a>
						        	<h5>
						        		<a href="{{ route('postDetail', ['unsigned_title'=>changeUrl($value->loaitin), 'id'=>$value->id]) }}" title="{{ cutTitle($value->title,15) }}">
						        			
						        		</a>
						        		<a href="{{ route('postDetail', ['unsigned_title'=>changeUrl($value->loaitin), 'id'=>$value->id]) }}">{{ cutTitle($value->title,15) }}
						        		</a>
						        	</h5>
						       	</a>
